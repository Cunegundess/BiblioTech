function excluirAutor(event, url) {
    ...
    if (confirm('Tem certeza de que deseja excluir este Autor?')) {
        ...
        .then(data => {
            ...
            window.location.href = data.redirect || 'http://127.0.0.1:8000/home/autores/';
        })
        .catch(error => {
...
function salvarAutor(event, url) {
    event.preventDefault();
    if (confirm('As alterações estão corretas?')) {
        // Obtenha os dados do formulário e converta para JSON
        const formData = new FormData(document.getElementById('editarAutorForm'));
        const jsonData = {};
