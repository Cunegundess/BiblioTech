function excluirEmprestimo(event, url) {
    event.preventDefault();
    if (confirm('Tem certeza de que deseja excluir este Emprestimo?')) {
        fetch(url, {
            method: "DELETE",
            headers: {
        ...
        .then(data => {
            ...
            window.location.href = data.redirect || 'http://127.0.0.1:8000/home/emprestimos/';
        })
        .catch(error => {
...
function salvarEmprestimo(event, url) {
    event.preventDefault();
    if (confirm('As alterações estão corretas?')) {
        // Obtenha os dados do formulário e converta para JSON
        const formData = new FormData(document.getElementById('editarEmprestimoForm'));
        const jsonData = {};
